...
func (r *RoleBindingReconciler) handleRoleBindingDeletion(ctx context.Context, rbClient *controllers2.RoleBindingClient, roleBinding *resourcev1alpha1.RoleBinding) (ctrl.Result, error) {
	...
	if controllerutil.ContainsFinalizer(roleBinding, roleBindingFinalizer) {
		...
		if err != nil {
			if !ignoreDeleteNotFound(logger, err, "Remote RoleBinding already deleted or not found", "roleBinding", roleBinding.Name) {
...
func (r *RoleBindingReconciler) findResourcesForConnection(ctx context.Context, obj client.Object) []ctrl.Request {
	resourceList := &resourcev1alpha1.RoleBindingList{}
